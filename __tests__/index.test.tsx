...
const reducerCreator = createReducerCreator<ITestState>({ id: 1, text: "init" })
  .case(setIdActionCreator, (state, action) => ({
    ...state,
    id: action.payload
  }))
  .case(setTextActionCreator, (state, action) => ({
    ...state,
...
const fooReducerCreator = createReducerCreator<{ foo: string }>({
  foo: ""
}).case(fooActionCreator, (state, action) => ({
  ...state,
  foo: action.payload
}));
...
describe("action", () => {
  ...
  it("setTextActionCreator({ text: 'test' }).payload toEqual { text: 'test' }", () => {
    expect(setTextActionCreator({ text: "test" }).payload).toEqual({
      text: "test"
    });
  });
});
...
describe("reducer", () => {
  it("reducer({ id: 1, text: 'test' }, setIdActionCreator(10))) toEqual { id: 10, text: 'test' }", () => {
    expect(reducer({ id: 1, text: "test" }, setIdActionCreator(10))).toEqual({
      id: 10,
      text: "test"
    });
  });
  ...
  it("reducer({ id: 1, text: 'test' }, setTextActionCreator({ text: 'changed!' }))) toEqual { id: 10, text: 'changed!' }", () => {
    expect(
      reducer(
        { id: 1, text: "test" },
        setTextActionCreator({ text: "changed!" })
      )
    ).toEqual({ id: 1, text: "changed!" });
  });

  it("reducer({ id: 1, text: 'test' }, addIdActionCreator({}))) toEqual { id: 2, text: 'test' }", () => {
    expect(reducer({ id: 1, text: "test" }, addIdActionCreator())).toEqual({
      id: 2,
      text: "test"
    });
  });
